...
def crop_image(image, position, size):
    """
    This function takes an image path, a position, and a size as input.
    It opens the image, crops it based on the given position and size,
    and returns the cropped image.

    :param image_path: The path to the image file.
    :param position: A tuple of two integers representing the top-left corner of the crop.
    :param size: An integer representing the width and height of the crop.
    ...
    cropped_image = image.crop((position[0], position[1], right, bottom))
...
def add_logo(background, logo, opaque_square, format_size, logo_size):

    x = opaque_square[0]
    y = format_size[1] - opaque_square[1] - logo_size[1]

    background = paste_img(background, logo, (x, y), logo_size)

def add_parteners_logos(background, partners, opaque_square, format_size, logo_size):
    if partners is None:
        return
                
    repertoire = f"{os.getcwd()}/assets/logos"

    x = format_size[0] - opaque_square[0] - logo_size[0]
    y = format_size[1] - opaque_square[1] - logo_size[1]

    for partner in partners:
        logo = load_image(f"{repertoire}/{partner}")
        background = paste_img(background, logo, (x, y), logo_size)
        x -= logo_size[0]
...
def add_event_elements(background, format, event, fonts_paths, theme, logo):
    ...
    ## LOGO & PARTENAIRES
    add_logo(background, logo, format.positions["opaque_square"], format.size, format.logo_size)
    add_parteners_logos(background, event.partners, format.positions["opaque_square"], format.size, format.logo_size)
...
def generate_event_presentation(event, save_path):
    ...
    description = write_event_presentation(event)
...
def write_association_presentation(association):

    presentation = f"Découvrez l'Association de l'{association.name}!\n\n"
    presentation += "Nos principaux objectifs sont "
    for i, obj in enumerate(association.object, 1):
        if i == len(association.object):
            presentation += f"{obj}.\n"
        else:
            presentation += f"{obj},\n"

    presentation += "\nNous sommes activement impliqués dans diverses activités telles que "
    for i, act in enumerate(association.month_events, 1):
        if i == len(association.month_events):
            presentation += f"{act}.\n"
        else:
            presentation += f"{act},\n"
...
def generate_association_presentation():
    repertoire = f"{os.getcwd()}/assets/results/PrésentationAssociation"

    presentation = write_association_presentation(Association.load_association())
    save_text(presentation, f"{repertoire}/presentation.txt")

def generate_all_events_publications():
    repertoire = os.getcwd()

    themes = Theme.load_themes()
    association = Association.load_association()
    ...
    for theme in themes:
        ...
        for format in formats:
            ...
            for i, bg in enumerate([back1, back2, back3, back4]):
                output_path = f"{repertoire}/assets/results/Themes/{theme.month}/{format.name}/{format.name}-{i}.png"
...
def generate_official(subjet, text):
    ...
    for format in formats:
        ...
        back2 = back1.copy()
        back2 = draw_transparent_square(back2, theme.getRGBcolor(), format.positions["opaque_square"])
        add_logo(back2, logo, format.positions["opaque_square"], format.size, format.logo_size)
        ...
        # SECOND
        add_text(back2, format.positions["title"], "L'Officiel", fonts_paths["title"], format.font_sizes["title"], theme.getRGBfont())
        formatted_text, text_x = format_official_text(ImageDraw.Draw(back2), text, format, fonts_paths["text"])
        print(f"{text_x}, {format.positions["description"][1]}")
